...
module.exports.onLoad = function () {
  ...
  const path = join(__dirname, "TAREK", "font");
  if (existsSync(path)) mkdirSync(path, { recursive: true });	

  const path2 = join(__dirname, "TAREK", "font");
...
________________________\n★যে্ঁকো্ঁনো্ঁ অ্ঁভি্ঁযো্ঁগ্ঁ অ্ঁথ্ঁবা্ৃ হে্ঁল্প্ঁ এ্ঁর্ঁ জ্ঁন্য্ঁ এ্ঁড্ঁমি্ঁন্ঁ 𝐓𝐀𝐑𝐄𝐊 কে্ঁ ন্ঁক্ঁ ক্ঁর্ঁতে্ঁ পা্ঁরে্ঁন্ঁ★\n𝐌𝐞𝐬𝐬𝐞𝐠𝐞𝐫:  https://m.me/tarek.20.king\n 𝐖𝐡𝐚𝐭𝐬𝐀𝐩𝐩: wa.me/+88017++++++36
⋆✦⋆⎯⎯⎯⎯⎯⎯⎯⎯⎯⋆✦⋆`, attachment: fs.createReadStream(__dirname + "/BADOL/join.mp4")} ,threadID));
    ...
    try {
      ...
      const randomPath = readdirSync(join(__dirname, "TAREK", "font"));

      if (existsSync(pathGif)) formPush = { body: msg, attachment: createReadStream(pathGif), mentions }
      else if (randomPath.length != 0) {
        const pathRandom = join(__dirname, "TAREK", "font", `${randomPath[Math.floor(Math.random() * randomPath.length)]}`);
